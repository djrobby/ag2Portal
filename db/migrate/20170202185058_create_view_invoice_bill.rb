class CreateViewInvoiceBill < ActiveRecord::Migration
  def up
    execute 'create view invoice_bills as
    ...
    where `invoices`.`invoice_operation_id`=1 group by `invoices`.`id`'
  end

  def down
    execute 'drop view invoice_bills'
  end
end
