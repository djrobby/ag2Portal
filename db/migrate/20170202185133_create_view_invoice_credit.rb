class CreateViewInvoiceCredit < ActiveRecord::Migration
  def up
    execute 'create view invoice_credits as
    ...
    where `invoices`.`invoice_operation_id`=2 group by `invoices`.`id`'
  end

  def down
    execute 'drop view invoice_credits'
  end
end
