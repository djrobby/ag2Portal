class CreateViewInvoiceDebt < ActiveRecord::Migration
  def up
    ...
    coalesce(sum(`client_payments`.`amount`),0) AS `paid`,
    (case when ((`invoices`.`discount_pct` = 0) or isnull(`invoices`.`discount_pct`)) then ((coalesce(sum((`invoice_items`.`quantity` * (`invoice_items`.`price` - `invoice_items`.`discount`))),0) + coalesce(sum(((`invoice_items`.`quantity` * (`invoice_items`.`price` - `invoice_items`.`discount`)) * (`tax_types`.`tax` / 100))),0)) - coalesce(sum(`client_payments`.`amount`),0)) else (((coalesce(sum((`invoice_items`.`quantity` * (`invoice_items`.`price` - `invoice_items`.`discount`))),0) - (coalesce(sum((`invoice_items`.`quantity` * (`invoice_items`.`price` - `invoice_items`.`discount`))),0) * (`invoices`.`discount_pct` / 100))) + (coalesce(sum(((`invoice_items`.`quantity` * (`invoice_items`.`price` - `invoice_items`.`discount`)) * (`tax_types`.`tax` / 100))),0) * (`invoices`.`discount_pct` / 100))) - coalesce(sum(`client_payments`.`amount`),0)) end) AS `debt`
    ...
    left join `client_payments` on((`client_payments`.`invoice_id` = `invoices`.`id`))) having (`debt` > 0)'
  end
