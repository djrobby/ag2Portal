/*
 * Methods to add 'fields' to the Office Notifications table 
 * Very important!!:
...
var of_notificationFieldsUI = {
    init: function(sel2NoMatches) {
        ...
        $('#addNotificationButton').on('click', function(e) {
            var isValid = $(of_n_cfg.formId).validate(false, validationSettings);
            if(!isValid) {
                e.stopPropagation();
                return false;
            }
            of_n_formHandler.appendFields(sel2NoMatches);
            of_n_formHandler.hideForm();
        });

        $('#cancelNotificationButton').on('click', function(e) {
          of_n_formHandler.removeFields();
          of_n_formHandler.hideForm();
        });
    }
};
...
var of_n_formHandler = {
    ...
    appendFields: function (sel2NoMatches) {
        ...
        var inputFields = $(of_n_cfg.formId + ' ' + of_n_cfg.inputFieldClassSelector);
        inputFields.detach();

        // Build the row and add it to the end of the table.
        of_n_rowBuilder.addRow(of_n_cfg.getTBodySelector(), inputFields);
    ...
    hideForm: function() {
        ...
        //$(of_n_cfg.tableId).trigger('totals');
        // Hide modal
        $(of_n_cfg.formId).modal('hide');
    }
};

var of_n_rowBuilder = function() {
