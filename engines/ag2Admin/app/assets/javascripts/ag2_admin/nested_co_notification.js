/*
 * Methods to add 'fields' to the Company Notifications table
 * Very important!!:
...
var co_notificationFieldsUI = {
    init: function(sel2NoMatches) {
        ...
        $('#addNotificationButton').on('click', function(e) {
            var isValid = $(co_n_cfg.formId).validate(false, validationSettings);
            if(!isValid) {
                e.stopPropagation();
                return false;
            }
            co_n_formHandler.appendFields(sel2NoMatches);
            co_n_formHandler.hideForm();
        });

        $('#cancelNotificationButton').on('click', function(e) {
          co_n_formHandler.removeFields();
          co_n_formHandler.hideForm();
        });
    }
};
...
var co_n_formHandler = {
    ...
    appendFields: function (sel2NoMatches) {
        ...
        var inputFields = $(co_n_cfg.formId + ' ' + co_n_cfg.inputFieldClassSelector);
        inputFields.detach();

        // Build the row and add it to the end of the table.
        co_n_rowBuilder.addRow(co_n_cfg.getTBodySelector(), inputFields);

        // Apply select2 to added row selects
        $('select.wsel2n').select2('destroy');
        $('select.wsel2n').select2({
    ...
    hideForm: function() {
        ...
        //$(co_n_cfg.tableId).trigger('totals');
        // Hide modal
        $(co_n_cfg.formId).modal('hide');
    }
};

var co_n_rowBuilder = function() {
    ...
    var buildRow = function(fields) {
        ...
        $(fields).map(function() {
            ...
            if (this.id.indexOf("s2") == -1) {
              // Apply CSS
              css = this.id;
              if ($(this).hasClass('fsel2')) css = css + ' select wsel2n';
              ...
              if (css.indexOf("wsel2n") == -1) css = css + ' sub-bordered-input';
              css = css + ' string';
