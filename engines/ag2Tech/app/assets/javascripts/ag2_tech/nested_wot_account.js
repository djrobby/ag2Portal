/*
 * Methods to add 'fields' to the Work order type Accounts table 
 * Very important!!:
...
var wot_accountFieldsUI = {
    init: function(sel2NoMatches) {
        ...
        $('#addAccountButton').on('click', function(e) {
            var isValid = $(wot_n_cfg.formId).validate(false, validationSettings);
            if(!isValid) {
                e.stopPropagation();
                return false;
            }
            wot_n_formHandler.appendFields(sel2NoMatches);
            wot_n_formHandler.hideForm();
        });

        $('#cancelAccountButton').on('click', function(e) {
          wot_n_formHandler.removeFields();
          wot_n_formHandler.hideForm();
        });
    }
};
...
var wot_n_formHandler = {
    ...
    appendFields: function (sel2NoMatches) {
        ...
        var inputFields = $(wot_n_cfg.formId + ' ' + wot_n_cfg.inputFieldClassSelector);
        inputFields.detach();

        // Build the row and add it to the end of the table.
        wot_n_rowBuilder.addRow(wot_n_cfg.getTBodySelector(), inputFields);
    ...
    hideForm: function() {
        ...
        //$(wot_n_cfg.tableId).trigger('totals');
        // Hide modal
        $(wot_n_cfg.formId).modal('hide');
    }
};

var wot_n_rowBuilder = function() {
