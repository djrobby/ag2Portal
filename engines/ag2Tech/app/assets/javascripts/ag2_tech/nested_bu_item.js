/*
 * Methods to add 'fields' to the Budget Items table 
 * Very important!!:
...
var bu_itemFieldsUI = {
    init: function(sel2NoMatches) {
        ...
        $('#addButton').on('click', function(e) {
            ...
            var isValid = $(bu_cfg.formId).validate(false, validationSettings);
            if(!isValid) {
                e.stopPropagation();
                return false;
            }
            bu_formHandler.appendFields(sel2NoMatches);
            bu_formHandler.hideForm();
        });

        $('#cancelButton').on('click', function(e) {
          bu_formHandler.removeFields();
          bu_formHandler.hideForm();
        });
    }
};
...
var bu_formHandler = {
    ...
    appendFields: function (sel2NoMatches) {
        ...
        var inputFields = $(bu_cfg.formId + ' ' + bu_cfg.inputFieldClassSelector);
        inputFields.detach();

        // Build the row and add it to the end of the table.
        bu_rowBuilder.addRow(bu_cfg.getTBodySelector(), inputFields);
    ...
    hideForm: function() {
        ...
        $(bu_cfg.tableId).trigger('totals');
        // Hide modal
        $(bu_cfg.formId).modal('hide');
    }
};

var bu_rowBuilder = function() {
    ...
    var buildRow = function(fields) {
        ...
        $(fields).map(function() {
            ...
            if (this.id.indexOf("s2") == -1) {
              // Setup new field(s)
              id = this.id;
              if ($(this).hasClass('fsel2') && id == 'fnt-account') {
