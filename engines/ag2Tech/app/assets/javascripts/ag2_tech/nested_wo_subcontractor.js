...
var wo_subcontractorFieldsUI = {
    init: function(sel2NoMatches) {
        var validationSettings = {
            errorMessagePosition : 'element'
        };

        $('#addSubcontractorButton').on('click', function(e) {
            var isValid = $(wo_s_cfg.formId).validate(false, validationSettings);
            if(!isValid) {
                e.stopPropagation();
                return false;
            }
            wo_s_formHandler.appendFields(sel2NoMatches);
            wo_s_formHandler.hideForm();
        });

        $('#cancelSubcontractorButton').on('click', function(e) {
          wo_s_formHandler.removeFields();
          wo_s_formHandler.hideForm();
        });
    }
};
...
var wo_s_cfg = {
    formId: '#new-subcontractor-fields',
    tableId: '#subcontractors-table',
...
var wo_s_formHandler = {
    ...
    appendFields: function (sel2NoMatches) {
        ...
        var inputFields = $(wo_s_cfg.formId + ' ' + wo_s_cfg.inputFieldClassSelector);
        inputFields.detach();

        // Build the row and add it to the end of the table.
        wo_s_rowBuilder.addRow(wo_s_cfg.getTBodySelector(), inputFields);

        // Apply select2 to added row selects
        $('select.ssel2').select2('destroy');
        $('select.ssel2').select2({
    ...
    hideForm: function() {
        ...
        $(wo_s_cfg.tableId).trigger('totals');
        // Hide modal
        $(wo_s_cfg.formId).modal('hide');
    }
};

var wo_s_rowBuilder = function() {
    ...
    var link = $('<a>', {
        href: '#',
        onclick: 'remove_fields(this); $("#subcontractors-table").trigger("totals"); return false;'
    ...
    var buildRow = function(fields) {
        ...
        $(fields).map(function() {
            ...
            if (this.id.indexOf("s2") == -1) {
              // Setup new field(s)
              id = this.id;
              if ($(this).hasClass('fsel2') && (id == 'fnt-supplier' || id == 'fnt-purchase-order')) {
...
/*
            ...
            if (this.id.indexOf("s2") == -1) {
              // Apply CSS
              id = this.id;
              if ($(this).hasClass('fsel2')) css = css + ' select ssel2';
              if ($(this).hasClass('number-text-field')) css = css + ' sub-number-text-field';
              if ($(this).hasClass('sub-disabled-field')) css = css + ' sub-disabled-field';
              if (css === '') css = css + ' sub-alfanumeric-text-field';
              if (css.indexOf("ssel2") == -1) css = css + ' sub-bordered-input';
