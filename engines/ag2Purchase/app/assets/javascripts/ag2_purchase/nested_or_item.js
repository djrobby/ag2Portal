/*
 * Methods to add 'fields' to the Offer Request Items table 
 * Very important!!:
...
var or_itemFieldsUI = {
    init: function(sel2NoMatches) {
        ...
        $('#addButton').on('click', function(e) {
            ...
            var isValid = $(or_cfg.formId).validate(false, validationSettings);
            if(!isValid) {
                e.stopPropagation();
                return false;
            }
            or_formHandler.appendFields(sel2NoMatches);
            or_formHandler.hideForm();
        });

        $('#cancelButton').on('click', function(e) {
          or_formHandler.removeFields();
          or_formHandler.hideForm();
        });
    }
};
...
var or_formHandler = {
    ...
    appendFields: function (sel2NoMatches) {
        ...
        var inputFields = $(or_cfg.formId + ' ' + or_cfg.inputFieldClassSelector);
        inputFields.detach();

        // Build the row and add it to the end of the table.
        or_rowBuilder.addRow(or_cfg.getTBodySelector(), inputFields);
    ...
    hideForm: function() {
        ...
        $(or_cfg.tableId).trigger('totals');
        // Hide modal
        $(or_cfg.formId).modal('hide');
    }
};

var or_rowBuilder = function() {
    ...
    var buildRow = function(fields) {
        ...
        $(fields).map(function() {
            ...
            if (this.id.indexOf("s2") == -1) {
              ...
              if (id === 'fnt-work-order' || id === 'fnt-project' ||
