/*
 * Methods to add 'fields' to the Offer Request Suppliers table 
 * Very important!!:
 *  1. remove_fields() & add_fields() are in main nested.js!!
 *  1. Modal window must be named (ie, 'new-supplier-fields')
 *  2. Items table must be named (ie. 'suppliers-table')
...
var or_supplierFieldsUI = {
    init: function(sel2NoMatches) {
        ...
        $('#addSupplierButton').on('click', function(e) {
            var isValid = $(or_w_cfg.formId).validate(false, validationSettings);
            if(!isValid) {
                e.stopPropagation();
                return false;
            }
            or_w_formHandler.appendFields(sel2NoMatches);
            or_w_formHandler.hideForm();
        });

        $('#cancelSupplierButton').on('click', function(e) {
          or_w_formHandler.removeFields();
          or_w_formHandler.hideForm();
        });
    }
};
...
var or_w_formHandler = {
    ...
    appendFields: function (sel2NoMatches) {
        ...
        var inputFields = $(or_w_cfg.formId + ' ' + or_w_cfg.inputFieldClassSelector);
        inputFields.detach();

        // Build the row and add it to the end of the table.
        or_w_rowBuilder.addRow(or_w_cfg.getTBodySelector(), inputFields);
    ...
    hideForm: function() {
        ...
        //$(or_w_cfg.tableId).trigger('totals');
        // Hide modal
        $(or_w_cfg.formId).modal('hide');
    }
};

var or_w_rowBuilder = function() {
