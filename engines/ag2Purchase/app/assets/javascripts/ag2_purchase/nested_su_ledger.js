...
var su_ledger_accountFieldsUI = {
    init: function(sel2NoMatches) {
        ...
        $('#addLedgerButton').on('click', function(e) {
            var isValid = $(su_ledger_cfg.formId).validate(false, validationSettings);
            if (!isValid) {
                e.stopPropagation();
                return false;
            }
            su_ledger_formHandler.appendFields(sel2NoMatches);
            su_ledger_formHandler.hideForm();
        });

        $('#cancelLedgerButton').on('click', function(e) {
          su_ledger_formHandler.removeFields();
          su_ledger_formHandler.hideForm();
        });
    }
};
...
var su_ledger_formHandler = {
    ...
    appendFields: function (sel2NoMatches) {
        ...
        var inputFields = $(su_ledger_cfg.formId + ' ' + su_ledger_cfg.inputFieldClassSelector);
        inputFields.detach();

        // Build the row and add it to the end of the table.
        su_ledger_rowBuilder.addRow(su_ledger_cfg.getTBodySelector(), inputFields);
    ...
    hideForm: function() {
        ...
        // Hide modal
        $(su_ledger_cfg.formId).modal('hide');
    }
};

var su_ledger_rowBuilder = function() {
