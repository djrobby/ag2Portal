...
module Ag2Gest
  class MeterTypesController < ApplicationController
    ...
    def index
      manage_filter_state
      @meter_types = MeterType.paginate(:page => params[:page], :per_page => 10).order(sort_column + ' ' + sort_direction)
    ...
    def update
      ...
      respond_to do |format|
        if @meter_type.update_attributes(params[:meter_type])
          format.html { redirect_to @meter_type, notice: t('activerecord.attributes.meter_type.successfully') }
