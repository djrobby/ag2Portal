...
module Ag2Gest
  class MeterLocationsController < ApplicationController
    ...
    def index
      manage_filter_state
      @meter_locations = MeterLocation.paginate(:page => params[:page], :per_page => 10).order(sort_column + ' ' + sort_direction)
    ...
    def update
      ...
      respond_to do |format|
        if @meter_location.update_attributes(params[:meter_location])
          format.html { redirect_to @meter_location, notice: t('activerecord.attributes.meter_location.successfully') }
